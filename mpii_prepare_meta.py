...
selected_joints = ["7", "8", "9", "10", "11", "12", "13", "14", "15"]
...
def save_joints(data_dir):
    label_file = os.path.join(data_dir, 'mpii_human_pose_v1_u12_2', 'mpii_human_pose_v1_u12_1.mat')
    joint_data_fn = os.path.join(data_dir, 'data.json')
        ...
        if 'annopoints' in str(anno['annorect'].dtype):
            ...
            for annopoint, head_x1, head_y1, head_x2, head_y2 in zip(
                ...
                if len(annopoint) > 0:
                    ...
                    is_include = len(set(selected_joints).intersection(set(joint_pos.keys()))) == len(selected_joints)
                    if is_include:
                        data = {
                            'filename': img_fn,
                            'train': train_flag,
...
def split_train_test(data_dir):
    fp_test = open(os.path.join(data_dir, 'test_joints.csv'), 'w')
    fp_train = open(os.path.join(data_dir, 'train_joints.csv'), 'w')
    all_data = open(os.path.join(data_dir, 'data.json'), 'r').readlines()
    N = len(all_data)
    N_test = int(N * 0.1)
    N_train = N - N_test
...
def main():
    ...
    save_joints(data_dir=args.data_dir)
